...
from utils import TooManyFramesError, TooMuchTimeError
...
class callgraph(object):
    ...
    def pretty_table(ignore_args_list=[]):
        TreePrinter().table(_tree, ignore_args_list=ignore_args_list, use_colors=True)

    @staticmethod
    def table_to_file(filename, ignore_args_list=[], show_argnames_in_call=False):
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(TreePrinter().table(_tree, ignore_args_list=ignore_args_list, show_argnames_in_call=show_argnames_in_call, use_colors=False))

    @staticmethod
    def graph_to_file(filename):
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(callgraph.get_graph_representation())


class node_data(object):
    def __init__(self, _args=None, _kwargs=None, _fn_name="", _arg_names=None):
        self.args = _args
        self.kwargs = _kwargs
...
def viz(arg_names=None):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if len(fullstack) > 2 and fullstack[2].function in ["__call__", "_execute", "wrapper"]:
            ...
            if this_frame_id not in g_callers.keys():
                g_callers[this_frame_id] = node_data(
                    copy.deepcopy(args), copy.deepcopy(kwargs), func.__name__, arg_names
                )
            ...
            if len(g_frames) > MAX_FRAMES:
                raise TooManyFramesError(
                    f"Encountered more than ${MAX_FRAMES} while executing function"
                )
            # Skip time check for now - would need a global start time tracking
    ...
    # Handle both @viz and @viz(arg_names=[...])
    if callable(arg_names):
        # Called as @viz (without parentheses)
        func = arg_names
        arg_names = None
        return decorator(func)
    else:
        # Called as @viz(...) (with parentheses)
        return decorator
